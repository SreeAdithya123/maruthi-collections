...
const FIELD =
  'w-full border bg-ivory px-3 py-2 font-sans text-sm text-ink outline-none transition-colors focus:border-zari-gold';
const LABEL = 'mb-1.5 block font-roman text-[0.55rem] uppercase tracking-[0.18em] text-ink-soft';

export default function Admin() {
  const { products, categories, addProduct, updateProduct, deleteProduct, resetCatalogue } = useProducts();
  const { user } = useAuth();
  const [editing, setEditing] = useState(null); // null | EMPTY-like form object
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-soft">
              Signed in as {user?.email} · {products.length} sarees · {categories.find((c) => c.slug === 'all')?.count} live
            </p>
          </div>
          <div className="flex gap-3">
            <button onClick={resetCatalogue} className="btn-ghost inline-flex items-center gap-2 !py-2.5">
